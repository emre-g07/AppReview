...
- (id)initWithFields:(NSArray *)fields {
    if (self = [super initWithFrame:CGRectMake(0.0f, 0.0f, [UIScreen mainScreen].bounds.size.width, 44.0f)])     {
        ...
        if (floor(NSFoundationVersionNumber) > NSFoundationVersionNumber_iOS_6_1) {
            ...
            [self setSegmentedControl:[[UISegmentedControl alloc] initWithItems:@[ UIU_LOCALIZED_STRING(@"PREVIOUS"),
                                                                                   UIU_LOCALIZED_STRING(@"NEXT") ]]];
        ...
        NSString *doneButtonText = @"OK";
        
        [self setDoneButton:[[UIBarButtonItem alloc] initWithTitle:doneButtonText
        ...
        [self setFields:[fields mutableCopy]];
        
        _leftArrowButton.accessibilityIdentifier = @"BSKeyboardControls_LeftArrowButton";
        _rightArrowButton.accessibilityIdentifier =  @"BSKeyboardControls_RightArrowButton";
        _doneButton.accessibilityIdentifier =  @"BSKeyboardControls_DoneButton";
    }
...
- (void)doneButtonPressed:(id)sender {
    for (id <BSKeyboardControlsDelegate> delegate in self.delegates) {
        if ([delegate respondsToSelector:@selector(keyboardControlsDonePressed:)]) {
...
- (void)selectPreviousField {
    ...
    if (index > 0) {
        ...
        for (id <BSKeyboardControlsDelegate> delegate in self.delegates) {
            if ([delegate respondsToSelector:@selector(keyboardControls:selectedField:inDirection:)]) {
                [delegate keyboardControls:self selectedField:field inDirection:BSKeyboardControlsDirectionPrevious];
            }
        }
    }
}
...
- (void)selectNextField {
    ...
    if (index < [self.fields count] - 1) {
        ...
        for (id <BSKeyboardControlsDelegate> delegate in self.delegates) {
            if ([delegate respondsToSelector:@selector(keyboardControls:selectedField:inDirection:)]) {
                [delegate keyboardControls:self selectedField:field inDirection:BSKeyboardControlsDirectionNext];
            }
        }
    }
}
...
- (void)addDelegate:(id<BSKeyboardControlsDelegate>)delegate {
    if (!self.delegates) {
